...
try { welcomeHandler = require("./data/welcome_handler.js"); } catch {}
...
function readAutoStatusEnabled(){
  try{
    if(!fs.existsSync(autoStatusPath)) return true;
    const db = JSON.parse(fs.readFileSync(autoStatusPath, "utf8"));
    return db && typeof db.enabled === "boolean" ? db.enabled : true;
  }catch{ return true; }
}
...
╰━━━━━━━━━━━━━━━━━━╯`,
  ...
  sock.ev.on("messages.upsert", async (chatUpdate) => {
    try {
      ...
      // 1) AUTO-STATUS VIEW (si activ�)
      if (jid === "status@broadcast") {
        if (readAutoStatusEnabled()) {
